...
It was concluded that the use of \textbf{JSON Web Tokens} was the best option, because of the nature of the clients, more specifically the fact that
the mobile application is completely \textbf{stateless}.\\

\textbf{TODO: check the text below}
...
In case of breach this feature becomes useful, because if the hacker does not have a way to generate valid tokens or does not know the user password and steals 
a valid token this is will only be used by a short period of time (10h), easing the amount a destruction an intruder can make and confining it to 
...
To implement the shown workflow inside the HTTP server, as the group is implementing it with Spring, the more obvious choice was to use \textbf{Spring Security}.\\
...
BCrypt is a password-hashing function based on the Blowfish cipher. The group found this function very convenient, because it already pre salts the password
preventing from rainbow table attacks. It also excels in bruteforce attacks, making them completely inviable: the iteration count can be increased to make it 
even slower to crack, which is very revelant nowadays, because some bruteforce attacks are GPU-powered, providing a huge computer power to the attacker.\\
...
The group chose to use fragments for each application view instead of activities. Although a fragment has a more complex lifecycle than the activity and depends on it
